...
local major = "DMWLibHealComm-4.0"
local minor = 89
...
local function parseHealEnd(casterGUID, pending, checkField, spellID, interrupted, ...)
	...
	if not interrupted then
		-- print("asd")
		HealAmount = (HealComm:GetCasterHealAmount(casterGUID, 1, GetTime() + 1) or 0) * (HealComm:GetHealModifier(casterGUID) or 1)
		-- HealComm:GetCasterHealAmount(casterGUID, 1, DMW.Time + 1)
	end
    -- print(HealAmount)
	local spellName = GetSpellInfo(spellID)
	...
	if HealAmount then
		-- print(HealAmount)
		HealComm.callbacks:Fire("HealComm_HealCustom", casterGUID, HealAmount,unpack(tempPlayerList))
	end
