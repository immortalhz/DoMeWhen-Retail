...
local MAJOR, MINOR = "LibCombatLogHealth-2.0", 11
...
function lib.UnitHealth(Pointer)
    local unit = DMW.Tables.Misc.pointer2unitFunc(Pointer)
    local clh = rawget(CLHealth, unit)
    if clh and clh[SYNC] then
        return clh[LOG_HEALTH][1]
    else
        return UnitHealth(Pointer)
    end
end
