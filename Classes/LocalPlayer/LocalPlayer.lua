...
function LocalPlayer:New(Pointer)
    ...
    self.SpecID = self.Level >= 10 and DMW.Enums.ClassSpec[GetSpecializationInfo(GetSpecialization())][2] or "LowLevel"
    self.Distance = 0
...
function LocalPlayer:Update()
    ...
    if self.Instance == "party" or self.Instance == "pvp" then
...
function LocalPlayer:GetSwing(hand)
    return DMW.Helpers.Swing.GetSwing(self.Pointer, hand)
end

function LocalPlayer:HasHeroism()
    for k in ipairs(DMW.Enums.HeroismBuff) do
        if LocalPlayer:AuraByID(k) then
            return true
        end
    end
end
...
local function ComputeRuneCooldown(Slot, BypassRecovery)
    ...
    -- Compute the CD.
    local CD = CDTime + CDValue - DMW.Time
    -- Return the Rune CD
